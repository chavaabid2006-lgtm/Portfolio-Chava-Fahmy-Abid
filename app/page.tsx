...
function SlideIn({ children, delay = 0, direction = "left" }: { children: React.ReactNode; delay?: number; direction?: string }) {
  const [ref, inView] = useInView();
...
function TagBadge({ tag }: { tag: string }) {
  ...
  return (
    <span className={`inline-block text-xs px-2.5 py-1 rounded-full border ${cls} font-medium tracking-wide`}>{tag}</span>
  );
}
...
const projects = [
  ...
  },
  {
    ...
    desc: [
      ...
      "The system improves issue tracking and organization by enabling efficient data storage, retrieval, and management.",
    ],
  ...
  },
  {
    ...
    desc: [
      ...
      "Led the development and management of the website/software component, implementing secure user authentication, real-time drone readings, and an AI-powered assistant feature.",
      "Worked closely with team members throughout the ideation, development, testing, and presentation phases.",
    ],
...
const certifications = [
  {
    ...
    typeColor: "amber",
    items: [{ name: "Python Advanced Level", date: "5 August 2024", icon: "🐍" }],
  },
  {
    provider: "BrightCHAMPS",
  ...
  return (
    <div ref={ref} style={{
      ...
      <div className={`rounded-2xl border overflow-hidden transition-all duration-300 ${project.highlight ? "border-amber-500/60 bg-gradient-to-br from-amber-950/30 to-zinc-900 shadow-lg shadow-amber-900/20" : "border-zinc-800 bg-zinc-900/60 hover:border-zinc-600"}`}>
        <button onClick={() => setOpen(!open)} className="w-full text-left p-4 sm:p-6 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="text-xl sm:text-2xl shrink-0">{project.icon}</span>
            <div>
              <h3 className={`text-base sm:text-xl font-semibold leading-snug ${project.highlight ? "text-amber-300" : "text-white"}`}>{project.title}</h3>
              {project.subtitle && <p className="text-amber-500/80 text-xs sm:text-sm mt-0.5">{project.subtitle}</p>}
            </div>
          </div>
          <span className={`text-zinc-400 text-xl transition-transform duration-300 shrink-0 ${open ? "rotate-45" : "rotate-0"}`}>+</span>
        </button>
        <div style={{ maxHeight: open ? "1000px" : "0", overflow: "hidden", transition: "max-height 0.5s cubic-bezier(0.4,0,0.2,1)" }}>
          <div className="px-4 sm:px-6 pb-5 border-t border-zinc-800/80 pt-4">
            <div className="space-y-3 text-zinc-300 text-sm leading-relaxed mb-4">
              {project.desc.map((p: string, i: number) => <p key={i}>{p}</p>)}
            </div>
            <div className="flex flex-wrap gap-1.5">
              {project.tags.map((tag: string) => <TagBadge key={tag} tag={tag} />)}
            </div>
          </div>
        </div>
      </div>
...
function SectionHeader({ label, title }: { label: string; title: string }) {
  return (
    <FadeIn>
      <span className="text-cyan-400 text-xs sm:text-sm tracking-widest uppercase font-medium">{label}</span>
      <h2 className="text-3xl sm:text-5xl font-bold mt-2 mb-8 sm:mb-12">{title}</h2>
    </FadeIn>
  );
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => { setScrolled(window.scrollY > 40); setMenuOpen(false); };
  ...
  return (
    <main className="bg-zinc-950 text-white min-h-screen overflow-x-hidden">
      ...
      {/* NAVBAR */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? "bg-zinc-950/95 backdrop-blur-xl border-b border-zinc-800/60" : "bg-transparent"}`}>
        {/* Desktop nav */}
        <div className="hidden md:flex max-w-5xl mx-auto justify-center gap-6 flex-wrap px-4 py-4">
          {navItems.map(item => (
        ...
        <div className="md:hidden flex items-center justify-between px-4 py-3">
          <span className="text-white text-sm font-semibold tracking-wide">Chava Fahmy Abid</span>
          <button onClick={() => setMenuOpen(!menuOpen)} className="text-zinc-400 hover:text-white p-1 bg-transparent border-none cursor-pointer">
        ...
        {/* Mobile dropdown */}
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${menuOpen ? "max-h-96" : "max-h-0"} bg-zinc-950/98 backdrop-blur-xl border-b border-zinc-800/60`}>
          <div className="px-4 py-3 grid grid-cols-2 gap-1">
            {navItems.map(item => (
              <button key={item} onClick={() => scrollTo(item.toLowerCase())}
                className="text-zinc-400 hover:text-cyan-400 text-xs tracking-widest uppercase py-2.5 text-left transition-colors duration-200 bg-transparent border-none cursor-pointer">
                {item}
              </button>
            ))}
          </div>
      ...
      {/* HERO */}
      <section className="relative h-screen flex flex-col justify-center items-center text-center px-5">
        <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? "translateY(0)" : "translateY(24px)", transition: "opacity 1s ease 0.2s, transform 1s ease 0.2s" }}>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-zinc-700 bg-zinc-900/80 mb-6">
            <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse shrink-0" />
            <span className="text-zinc-400 text-xs sm:text-sm tracking-wider">Available for opportunities</span>
          </div>
        </div>
        <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? "translateY(0)" : "translateY(32px)", transition: "opacity 1s ease 0.4s, transform 1s ease 0.4s" }}>
          <h1 className="text-5xl sm:text-7xl md:text-8xl font-bold tracking-tight mb-4" style={{ background: "linear-gradient(135deg, #ffffff 0%, #94a3b8 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            Chava Fahmy<br />Abid
          </h1>
        </div>
        ...
        <div style={{ opacity: heroVisible ? 1 : 0, transition: "opacity 1s ease 1s" }} className="absolute bottom-8 left-1/2 -translate-x-1/2">
          <div className="w-px h-12 mx-auto bg-gradient-to-b from-zinc-600 to-transparent animate-pulse" />
        </div>
      </section>
      ...
      {/* ABOUT */}
      <section id="about" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
        ...
        <div className="grid md:grid-cols-2 gap-6 sm:gap-10 mb-10 sm:mb-16">
          ...
          <SlideIn delay={0.2} direction="right">
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 divide-y divide-zinc-800">
              {[
                ...
                <div key={label} className="flex justify-between items-start py-2.5 gap-3">
                  ...
                  <span className="text-zinc-200 text-xs sm:text-sm text-right">{value}</span>
                </div>
              ))}
            </div>
          </SlideIn>
            ...
            {[
              ...
              <div key={label} className="flex gap-3 p-3 sm:p-4 rounded-xl border border-zinc-800 bg-zinc-900/40 hover:border-zinc-600 transition-colors duration-200">
                ...
                <div>
                  ...
                  <p className="text-zinc-200 text-xs sm:text-sm leading-relaxed">{value}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <FadeIn delay={0.35}>
          <div className="grid sm:grid-cols-2 gap-4 mt-8 sm:mt-10">
            {/* Sports */}
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 hover:border-zinc-600 transition-colors duration-300">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-rose-950/70 border border-rose-800/60 flex items-center justify-center text-base">🥊</div>
                <h3 className="text-white font-semibold text-base sm:text-lg">Sports</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {[{ name: "Muaythai", icon: "🥊" }, { name: "Kickboxing", icon: "🦵" }, { name: "Boxing", icon: "🥋" }, { name: "Basketball", icon: "🏀" }, { name: "Table Tennis", icon: "🏓" }, { name: "Swimming", icon: "🏊" }].map(({ name, icon }) => (
                  <span key={name} className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full border bg-rose-950/30 text-rose-300 border-rose-800/60 font-medium">
                    <span>{icon}</span>{name}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 hover:border-zinc-600 transition-colors duration-300">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-purple-950/70 border border-purple-800/60 flex items-center justify-center text-base">🎯</div>
                <h3 className="text-white font-semibold text-base sm:text-lg">Hobbies</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {[{ name: "Chess", icon: "♟️" }, { name: "Cafe Hopping", icon: "☕" }, { name: "Fashion", icon: "👔" }, { name: "Strategy Gaming", icon: "🎮" }, { name: "Cinema", icon: "🎬" }, { name: "Marvel Comics", icon: "📚" }].map(({ name, icon }) => (
                  <span key={name} className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full border bg-purple-950/30 text-purple-300 border-purple-800/60 font-medium">
                    <span>{icon}</span>{name}
                  </span>
                ))}
              </div>
      ...
      {/* EDUCATION */}
      <section id="education" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
        <SectionHeader label="Background" title="Education" />
        <div className="space-y-4">
          <FadeIn delay={0.1}>
            <div className="relative rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 sm:p-8 overflow-hidden hover:border-zinc-600 transition-colors duration-300">
              ...
              <div className="pl-4 sm:pl-6">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <h3 className="text-lg sm:text-2xl font-bold text-white">Universiti Teknologi PETRONAS</h3>
                    <p className="text-zinc-400 mt-1 text-sm">Malaysia</p>
                  </div>
                  <span className="text-zinc-500 text-xs sm:text-sm">September 2024 – Present</span>
                </div>
                <p className="text-cyan-300 font-medium mt-3 text-sm sm:text-base">Bachelor of Information Technology (Hons)</p>
                <p className="text-zinc-500 text-xs sm:text-sm mt-1">Current Semester: 5th</p>
                <div className="flex flex-wrap gap-2 sm:gap-3 mt-4">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-cyan-950/50 border border-cyan-800/60">
                    <span className="text-cyan-400 text-xs uppercase tracking-wider font-medium">CGPA</span>
                    <span className="text-white font-bold text-sm">3.32</span>
                  </div>
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-amber-950/40 border border-amber-800/50 flex-wrap">
                    <span className="text-amber-400 text-xs uppercase tracking-wider font-medium">Dean&apos;s List</span>
                    <span className="text-white font-bold text-sm">January 2025</span>
                    <span className="text-zinc-400 text-xs">· GPA 3.64</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <FadeIn delay={0.2}>
            <div className="relative rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 sm:p-8 overflow-hidden hover:border-zinc-600 transition-colors duration-300">
              ...
              <div className="pl-4 sm:pl-6">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <h3 className="text-lg sm:text-2xl font-bold text-white">SMA Swasta Batari Polonia</h3>
                    <p className="text-zinc-400 mt-1 text-sm">Medan, Indonesia</p>
                  </div>
                  <span className="text-zinc-500 text-xs sm:text-sm">June 2021 – May 2024</span>
                </div>
                <p className="text-violet-300 font-medium mt-3 text-sm sm:text-base">Senior High School</p>
                <p className="text-zinc-500 text-xs sm:text-sm mt-1">Mathematics and Science Program</p>
                <div className="flex flex-wrap gap-2 mt-4">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-violet-950/50 border border-violet-800/60">
                    <span className="text-violet-400 text-xs uppercase tracking-wider font-medium">Average Grade</span>
                    <span className="text-white font-bold text-sm">89.68</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* SKILLS */}
      <section id="skills" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
        <SectionHeader label="What I bring" title="Skills" />
        <div className="grid md:grid-cols-3 gap-4 sm:gap-5">
          {/* Languages */}
          <FadeIn delay={0.05}>
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 hover:border-zinc-600 transition-colors duration-300 h-full">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-xl bg-cyan-950/70 border border-cyan-800/60 flex items-center justify-center text-base">🌐</div>
                <h3 className="text-white font-semibold text-base sm:text-lg">Languages</h3>
              </div>
                ...
                {[
                  ...
                  <div key={lang}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-zinc-200 text-xs sm:text-sm">{lang}</span>
          ...
          <FadeIn delay={0.1}>
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 hover:border-zinc-600 transition-colors duration-300 h-full">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-xl bg-violet-950/70 border border-violet-800/60 flex items-center justify-center text-base">💻</div>
                <h3 className="text-white font-semibold text-base sm:text-lg">Computer</h3>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {["MS Word", "MS Excel", "PowerPoint", "Google Docs", "Google Sheets", "Google Drive", "Zoom", "MS Teams", "Canva", "Figma"].map(t => (
                  <span key={t} className="text-xs px-2 py-1 rounded-full border bg-zinc-800/80 text-zinc-300 border-zinc-700">{t}</span>
                ))}
              </div>
              <p className="text-zinc-500 text-xs uppercase tracking-wider mb-2">Programming & Technical</p>
              <div className="flex flex-wrap gap-1.5">
          ...
          <FadeIn delay={0.15}>
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 hover:border-zinc-600 transition-colors duration-300 h-full">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-xl bg-emerald-950/70 border border-emerald-800/60 flex items-center justify-center text-base">🤝</div>
                <h3 className="text-white font-semibold text-base sm:text-lg">Interpersonal</h3>
              </div>
              <div className="space-y-2">
                {[
                  ...
                  <div key={skill} className="flex items-center gap-3 p-2 sm:p-2.5 rounded-xl bg-zinc-800/50 border border-zinc-700/50">
                    <span className="text-sm">{icon}</span>
                    <span className="text-zinc-200 text-xs sm:text-sm">{skill}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* PROJECTS */}
      <section id="projects" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
      ...
      {/* ACHIEVEMENTS */}
      <section id="achievements" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
        <SectionHeader label="Recognition" title="Achievements" />

        <FadeIn delay={0.05}>
          <div className="flex items-center gap-3 mb-5">
            <div className="h-px flex-1 bg-zinc-800" />
            <span className="text-zinc-500 text-xs uppercase tracking-widest text-center">Universiti Teknologi PETRONAS</span>
            <div className="h-px flex-1 bg-zinc-800" />
          </div>
        </FadeIn>
        ...
        <div className="space-y-3 mb-10">
          <FadeIn delay={0.1}>
            <div className="relative rounded-2xl border border-cyan-800/50 bg-cyan-950/20 p-4 sm:p-6 hover:border-cyan-600/60 transition-colors duration-300">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="flex items-start gap-3">
                  <span className="text-xl mt-0.5">🎓</span>
                  <div>
                    <p className="text-white font-semibold text-base sm:text-lg">Dean&apos;s List</p>
                    <p className="text-cyan-400 text-xs sm:text-sm mt-0.5">Universiti Teknologi PETRONAS</p>
                  </div>
                </div>
                <span className="text-zinc-500 text-xs sm:text-sm">January 2025</span>
              </div>
              <p className="text-zinc-400 text-xs sm:text-sm mt-3 pl-9">GPA 3.64 — Academic excellence recognition for outstanding semester performance.</p>
            </div>
          </FadeIn>
          <FadeIn delay={0.15}>
            <div className="relative rounded-2xl border border-amber-600/50 bg-amber-950/20 p-4 sm:p-6 hover:border-amber-500/60 transition-colors duration-300">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="flex items-start gap-3">
                  <span className="text-xl mt-0.5">🏆</span>
                  <div>
                    <p className="text-white font-semibold text-base sm:text-lg">4th Place — Invent for the Planet 2025</p>
                    <p className="text-amber-400 text-xs sm:text-sm mt-0.5">UTP & Texas A&amp;M University · 9 February 2025</p>
                  </div>
                </div>
              </div>
              <p className="text-zinc-400 text-xs sm:text-sm mt-3 pl-9">Developed and managed a website/software system integrated with drone hardware. Built a secure login system, displayed real-time drone readings, and integrated an AI-powered assistant feature for users.</p>
            </div>
          </FadeIn>
        </div>
        ...
        <FadeIn delay={0.2}>
          <div className="flex items-center gap-3 mb-5">
            <div className="h-px flex-1 bg-zinc-800" />
            <span className="text-zinc-500 text-xs uppercase tracking-widest text-center">SMA Swasta Batari Polonia, Medan</span>
            <div className="h-px flex-1 bg-zinc-800" />
          </div>
        </FadeIn>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 sm:p-6 hover:border-zinc-600 transition-colors duration-300">
            <div className="divide-y divide-zinc-800/80">
              {[
                ...
                <div key={award} className="flex items-start justify-between gap-3 py-2.5">
                  <div className="flex items-start gap-2.5">
                    <span className="text-sm shrink-0 mt-0.5">{icon}</span>
                    <span className="text-zinc-200 text-xs sm:text-sm">{award}</span>
                  </div>
      ...
      {/* EXPERIENCE */}
      <section id="experience" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
        <SectionHeader label="Involvement" title="Experience" />
        ...
        <FadeIn delay={0.1}>
          <div className="relative rounded-2xl border border-emerald-700/50 bg-emerald-950/20 p-4 sm:p-6 hover:border-emerald-500/60 transition-colors duration-300">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="flex items-start gap-3">
                <span className="text-xl mt-0.5">☀️</span>
                <div>
                  <p className="text-white font-semibold text-base sm:text-lg">Treasurer (HICOM)</p>
                  <p className="text-emerald-400 text-xs sm:text-sm mt-0.5">Native Power Project · MPU4 Community Project</p>
                </div>
              </div>
              <span className="text-zinc-500 text-xs sm:text-sm shrink-0">25–26 April 2026</span>
            </div>
            ...
            <div className="mt-4 pl-9 flex flex-wrap gap-1.5">
              {["Community Service", "Project Management", "Treasurer", "Solar Energy", "Rural Development"].map(tag => (
                <span key={tag} className="inline-block text-xs px-2.5 py-1 rounded-full border bg-emerald-950/40 text-emerald-300 border-emerald-800/60 font-medium">{tag}</span>
              ))}
            </div>
          </div>
        </FadeIn>
        ...
        <div className="space-y-4">
          {certifications.map((group, gi) => (
            <FadeIn key={group.type} delay={0.1 + gi * 0.1}>
              <div className={`rounded-2xl border overflow-hidden transition-colors duration-300 ${group.typeColor === "amber" ? "border-amber-600/50 bg-amber-950/20 hover:border-amber-500/60" : "border-cyan-800/50 bg-cyan-950/20 hover:border-cyan-600/60"}`}>
                <div className="px-4 sm:px-6 pt-4 pb-3 flex items-center gap-3 border-b border-zinc-800/60">
                  <span className="text-xl">{group.typeColor === "amber" ? "🏅" : "📜"}</span>
                  <div>
                    <p className={`font-semibold text-sm sm:text-base ${group.typeColor === "amber" ? "text-amber-300" : "text-cyan-300"}`}>{group.type}</p>
                    <p className="text-zinc-500 text-xs mt-0.5">{group.provider}</p>
                  </div>
                </div>
                <div className="divide-y divide-zinc-800/60">
                  {group.items.map(({ name, date, icon }) => (
                    <div key={name} className="flex items-center justify-between gap-3 px-4 sm:px-6 py-3">
                      <div className="flex items-center gap-2.5">
                        <span className="text-base shrink-0">{icon}</span>
                        <span className="text-zinc-200 text-xs sm:text-sm font-medium">{name}</span>
                      </div>
      ...
      {/* CONTACT */}
      <section id="contact" className="max-w-5xl mx-auto py-16 sm:py-28 px-4 sm:px-6">
          ...
          {[
            ...
            <FadeIn key={label} delay={delay}>
              <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer"
                className="flex gap-3 items-start p-3 sm:p-4 rounded-xl border border-zinc-700 bg-zinc-900/60 hover:border-cyan-600 hover:bg-cyan-950/20 transition-all duration-300 group">
                <span className="text-base mt-0.5 shrink-0">{icon}</span>
                <div>
                  <p className="text-zinc-500 text-xs uppercase tracking-wider mb-1">{label}</p>
                  <p className="text-zinc-300 group-hover:text-white text-xs sm:text-sm transition-colors break-all">{value}</p>
                </div>
              </a>
            </FadeIn>
          ))}
      ...
      <footer className="border-t border-zinc-800/60 py-6 text-center text-zinc-600 text-xs sm:text-sm">
